/* @ts-self-types="./apalabrar_editor_core.d.ts" */
 ...
 * @param {number} end
 */
export function applyDelete(doc_id, start, end) {
    const ret = wasm.applyDelete(doc_id, start, end);
    if (ret[1]) {
 ...
 * @param {string} text
 */
export function applyInsert(doc_id, offset, text) {
    const ptr0 = passStringToWasm0(text, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.applyInsert(doc_id, offset, ptr0, len0);
    if (ret[1]) {
        throw takeFromExternrefTable0(ret[0]);
    }
}
 ...
 * @returns {string}
 */
export function docText(doc_id) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ret = wasm.docText(doc_id);
        var ptr1 = ret[0];
        var len1 = ret[1];
        if (ret[3]) {
            ptr1 = 0; len1 = 0;
...
export function openDocx(bytes) {
    const ptr0 = passArray8ToWasm0(bytes, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.openDocx(ptr0, len0);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return BigInt.asUintN(64, ret[0]);
 ...
 * @returns {Uint8Array}
 */
export function toDocx(doc_id) {
    const ret = wasm.toDocx(doc_id);
    if (ret[3]) {
        throw takeFromExternrefTable0(ret[2]);
    }
    var v1 = getArrayU8FromWasm0(ret[0], ret[1]).slice();
...
function __wbg_get_imports() {
    const import0 = {
        ...
        __wbg_now_f462ca322babb89f: function() {
            const ret = Date.now();
            return ret;
        },
    ...
    return {
        __proto__: null,
        "./apalabrar_editor_core_bg.js": import0,
    };
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('apalabrar_editor_core_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
